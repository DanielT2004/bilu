...
struct HomeView: View {
    ...
    private var mapWidget: some View {
        ZStack(alignment: .bottom) {
            // Map background
            Rectangle().fill(Color(hex: "e4ddd0"))

            // Grid lines canvas
            Canvas { ctx, size in
                let hLines: [CGFloat] = [0.30, 0.60, 0.84]
                let vLines: [CGFloat] = [0.22, 0.44, 0.66, 0.84]
                let strokeColor = GraphicsContext.Shading.color(Color(hex: "cac3b5"))
                for pct in hLines {
                    var p = Path(); p.move(to: CGPoint(x: 0, y: size.height * pct))
                    p.addLine(to: CGPoint(x: size.width, y: size.height * pct))
                    ctx.stroke(p, with: strokeColor, lineWidth: 0.5)
                }
                for pct in vLines {
                    var p = Path(); p.move(to: CGPoint(x: size.width * pct, y: 0))
                    p.addLine(to: CGPoint(x: size.width * pct, y: size.height))
                    ctx.stroke(p, with: strokeColor, lineWidth: 0.5)
                }
            }

            // Center location pin (positioned above the bar)
            VStack {
                Spacer()
                Circle()
                    .fill(C.green)
